...
export default function AdminListItem({ title, subtitle, onDelete, onEdit }) {
  return (
    <div className="flex items-center justify-between gap-3 bg-white border-2 border-ink rounded-md px-4 py-2.5 shadow-[2px_2px_0_#1A1A2E]">
      ...
      <div className="flex items-center gap-2">
        {onEdit && (
          <button
            onClick={onEdit}
            className="flex-shrink-0 flex items-center gap-1 font-display text-xs px-2.5 py-1 bg-yellow text-ink border-2 border-ink rounded-md shadow-[2px_2px_0_#1A1A2E] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[1px_1px_0_#1A1A2E] transition-all"
          >
